import{_ as s,o as n,c as e,Q as a}from"./chunks/framework.0f4e1e9e.js";const l="/static/1.fba0983b.jpg",p="/static/2.5dc5fc39.jpg",t="/static/3.17dbc5c1.jpg",o="/static/4.b3b2914a.jpg",v=JSON.parse('{"title":"ESLint 实践","description":"","frontmatter":{},"headers":[],"relativePath":"frontend/1 前端工程化/3 开发/3 规范化/ESLint+Prettier/ESLint实践.md","filePath":"frontend/1 前端工程化/3 开发/3 规范化/ESLint+Prettier/ESLint实践.md"}'),i={name:"frontend/1 前端工程化/3 开发/3 规范化/ESLint+Prettier/ESLint实践.md"},r=a(`<h1 id="eslint-实践" tabindex="-1">ESLint 实践 <a class="header-anchor" href="#eslint-实践" aria-label="Permalink to &quot;ESLint 实践&quot;">​</a></h1><h2 id="简介" tabindex="-1">简介 <a class="header-anchor" href="#简介" aria-label="Permalink to &quot;简介&quot;">​</a></h2><p>ESLint 是一个检查代码质量与风格的工具，配置一套规则，就能检查出你代码中不符合规则的地方，部分问题支持自动修复；</p><p>总结起来就两个功能：</p><ul><li>代码质量检查 <ul><li>可以发现代码中存在的可能错误，如使用未声明变量、声明而未使用的变量、修改 const 变量、代码中使用debugger等等</li></ul></li><li>代码格式化 <ul><li>可以用来统一团队的代码风格，比如加不加分号、使用 tab 还是空格、字符串使用单引号 等等</li></ul></li></ul><p>ESLint是一个插件化的代码检测工具，正如它官网描述的slogan：</p><blockquote><p>可组装的JavaScript和JSX检查工具</p></blockquote><p>ESLint不仅可以检测JS，还支持JSX和Vue，它的高可扩展性让它能够支持更多的项目。</p><p>刚开始ESlint的推出并没有撼动JSHint的霸主地位，由于ESlint需要将源码转为AST，而JSHint直接检测源文件字符串，因此执行速度比JSHint慢很多；真正让ESLint实现弯道超车的是ES6的出现。</p><p>2015年，ES6规范发布后，由于大部分浏览器支持程度不高，因此需要Babel将代码转换编译成ES5或者更低版本；同时由于ES6变化很大，短期内JSHint无法完全支持，这时ESLint的高扩展性的优点显现出来了，不仅可以扩展规则，连默认的解析器也能替换；Babel团队就为ESLint开发了babel-eslint替换默认的解析器esprima，让ESLint率先支持ES6。</p><p>ESLint扩展性好；ESLint的核心是检测AST是否符合规则；</p><ol><li>第一步：解析器把源码转成AST；ESLint的解析器可自定义，源码用了啥高级语法或者换成TS，可直接换解析器得到最终的AST，eslint就可做规则匹配了；</li><li>第二步：执行规则；ESLint的检测规则可自定义；</li></ol><h2 id="安装" tabindex="-1">安装 <a class="header-anchor" href="#安装" aria-label="Permalink to &quot;安装&quot;">​</a></h2><p>npm i eslint --save-dev</p><p>npx eslint --init // 生成 .eslintrc* 配置文件</p><p>一般我们会把eslint命令行配置到packages.json中：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#e1e4e8;">&quot;scripts&quot;: {</span></span>
...
<span class="line"><span style="color:#24292e;">},</span></span></code></pre></div><p>但是像浏览器环境或者node环境全局变量有很多，一个个指定又太麻烦了，此时就可以使用env去指定环境，这样我们就无需一个个指定这个环境下的全局变量了</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#e1e4e8;">{</span></span>
...
<span class="line"><span style="color:#24292e;">}</span></span></code></pre></div><h3 id="parser和parseroptions" tabindex="-1">parser和parserOptions <a class="header-anchor" href="#parser和parseroptions" aria-label="Permalink to &quot;parser和parserOptions&quot;">​</a></h3><p>eslint默认的解析器是espree，只支持转换js，默认支持到ES5的语法，假如需要支持更高版本的语法，则可以通过parserOptions来指定版本号</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#e1e4e8;">{</span></span>
