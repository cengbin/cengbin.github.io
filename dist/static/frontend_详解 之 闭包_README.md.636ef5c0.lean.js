import{_ as n,o as a,c as e,O as l}from"./chunks/framework.d9e2f5d0.js";const p="/static/1.4aa2464a.jpg",o="/static/2.b77f116d.jpg",g=JSON.parse('{"title":"深入理解 JavaScript 闭包","description":"","frontmatter":{},"headers":[],"relativePath":"frontend/详解 之 闭包/README.md","filePath":"frontend/详解 之 闭包/README.md"}'),r={name:"frontend/详解 之 闭包/README.md"};function t(c,s,i,u,d,y){return a(),e("div",null,s[0]||(s[0]=[l(`<h1 id="深入理解-javascript-闭包" tabindex="-1">深入理解 JavaScript 闭包 <a class="header-anchor" href="#深入理解-javascript-闭包" aria-label="Permalink to &quot;深入理解 JavaScript 闭包&quot;">​</a></h1><h2 id="_1-前言" tabindex="-1">1. 前言 <a class="header-anchor" href="#_1-前言" aria-label="Permalink to &quot;1. 前言&quot;">​</a></h2><p>面试的时候常常会有面试官会问：谈谈你对闭包的理解？</p><p>首先回答问题要有清晰的思路和逻辑顺序，如：</p><ol><li>描述问题产生的背景。</li><li>如何解决的问题。</li><li>有什么结果。</li></ol><p>比如回答对闭包的理解可以按：</p><ol><li>为什么会有闭包？</li><li>闭包是如何设计的？</li><li>闭包的应用场景。</li></ol><p>我觉得按这样的思路回答问题，比按<a href="https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Closures" target="_blank" rel="noreferrer">MDN</a>上的解释说一遍回答“闭包（closure）是一个函数以及其捆绑的周边环境状态（lexical environment，词法环境）的引用的组合”要好。</p><h2 id="_2-为什么会有闭包" tabindex="-1">2. 为什么会有闭包？ <a class="header-anchor" href="#_2-为什么会有闭包" aria-label="Permalink to &quot;2. 为什么会有闭包？&quot;">​</a></h2><p>在<a href="http://en.wikipedia.org/wiki/Stack-oriented_programming_language" target="_blank" rel="noreferrer">面向堆栈的编程语言</a>中，函数的局部变量都是保存在栈上的，每当函数激活的时候，这些变量和函数参数都会压入到该堆栈上。</p><p>当函数返回的时候，这些参数又会从栈中移除。这种模型对将函数作为函数式值使用的时候有很大的限制（比方说，作为返回值从父函数中返回）。绝大部分情况下，问题会出现在当函数有自由变量的时候。</p><p>在ECMAScript中，函数是可以封装在父函数中的，并<strong>可以使用父函数上下文的变量</strong>。这个特性会引发Funarg问题。</p><h3 id="funarg-问题" tabindex="-1">Funarg 问题 <a class="header-anchor" href="#funarg-问题" aria-label="Permalink to &quot;Funarg 问题&quot;">​</a></h3><p>自由变量是指<strong>在函数中使用</strong>的，但既<strong>不是函数参数</strong>也<strong>不是函数的局部变量</strong>的变量</p><p>示例1：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#e1e4e8;">  function foo() {</span></span>
...
<span class="line"><span style="color:#24292e;">}</span></span></code></pre></div><p>函数在创建阶段会给foo函数创建[[Scopes]]属性</p><p>函数执行上下文作用域链： fooContext.Scope = fooContext.AO + foo.[[Scopes]]</p><p>技术上说，创建该函数的父级上下文的数据是保存在该函数的内部属性 [[Scopes]]中的。</p><p>但是 JS 引擎怎么知道它要用到哪些外部引用呢，需要做 AST 扫描，很多 JS 引擎会做 Lazy Parsing，这时候去 parse 函数，正好也能知道它用到了哪些外部引用，然后把这些外部用打包成 Closure 闭包，加到 [[scopes]] 中。</p><p>如下图：firstClosure函数在创建完成之后，函数内部引用的自由变量就已经打包成Closure闭包，挂到函数的[[Scopes]]上了。</p><p><img src="`+p+'" alt="1.png"></p><p>调用 firstClosure 函数的时候，JS 引擎 会取出 [[Scopes]] 中的打包的 Closure + Global 链，设置成新的作用域链， 这就是函数用到的所有外部环境了，有了外部环境，自然就可以运行了。</p><p><img src="'+o+'" alt="2.jpg"></p><h2 id="_4-闭包的应用场景" tabindex="-1">4. 闭包的应用场景 <a class="header-anchor" href="#_4-闭包的应用场景" aria-label="Permalink to &quot;4. 闭包的应用场景&quot;">​</a></h2><ul><li>命名空间污染：模块要用多个变量，我们希望变量不影响全局，全局也不影响我们的变量。</li><li>模拟私有属性</li><li>有状态的函数</li></ul><h2 id="_5-闭包的缺点" tabindex="-1">5. 闭包的缺点 <a class="header-anchor" href="#_5-闭包的缺点" aria-label="Permalink to &quot;5. 闭包的缺点&quot;">​</a></h2><ul><li>如果一个变量被闭包对象 Closure 引用，无法被释放回收。</li><li>如果一个很大的对象被闭包对象 Closure 引用，本来函数调用结束就能销毁，但是现在引用却被通过闭包保存到了堆里，而且还一直用不到，那这块堆内存就一直没法使用，严重到一定程度就算是内存泄漏了。</li><li>闭包函数又在多个地方被引用，导致数据引用复杂，容易发生内存泄漏问题。</li></ul><p>至此通过清晰的回答，面试官会对你肯定刮目相看。</p><p>参考：</p><p><a href="https://juejin.cn/post/7084549768067678245?share_token=81B8021F-F924-477C-AC49-47071C69927E" target="_blank" rel="noreferrer">闭包是什么？从为什么会有闭包讲起！</a></p>',42)]))}const f=n(r,[["render",t]]);export{g as __pageData,f as default};
