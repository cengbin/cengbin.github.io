...
<span class="line"><span style="color:#24292e;">		&lt;/script&gt;</span></span>
<span class="line"><span style="color:#24292e;">	&lt;/body&gt;</span></span>
<span class="line"><span style="color:#24292e;">&lt;/html&gt;</span></span></code></pre></div><p><strong>音频可视化</strong></p><h5 id="创建analysernode对象" tabindex="-1">创建AnalyserNode对象 <a class="header-anchor" href="#创建analysernode对象" aria-label="Permalink to &quot;创建AnalyserNode对象&quot;">​</a></h5><p><img src="`+r+`" alt=""></p><ul><li>AudioContext 的 createAnalyser（）方法能创建一个 AnalyserNode，可以用来获取音频时间和频率数据，以及实现数据可视化。</li></ul><p><code>const analyser = ctx.createAnalyser();</code></p><h5 id="设置-fftsize-属性" tabindex="-1">设置 fftSize 属性 <a class="header-anchor" href="#设置-fftsize-属性" aria-label="Permalink to &quot;设置 fftSize 属性&quot;">​</a></h5><ul><li>AnalyserNode 接口的 fftsize 属性的值是一个无符号长整型的值，表示（信号）样本的窗口大小。当执行快速傅里叶变换（Fast FourierTransfor （FFT））时，这些（信号）样本被用来获取频域数据。</li><li>fftSize 属性的值必须是从32到32768范围内的2的非零幂；其默认值为2048。</li></ul><p><code>analyser. fftSize = 512;</code></p><ul><li>定义一个8位无符号整型数组，他的长度为波形的一半，因为波形的前后一半和后一半是相同的，所以获取一半即可此处用的是analyser中的方法，便于理解也可以使用analyser.fftSize/2结果是一样的</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#e1e4e8;">const bufferLength = analyserNode.frequencyBinCount</span></span>
